...
def test_get_customers(authorized_client, test_customers: list[Customer]):
    ...
    assert (
        total_pages == len(test_customers) // len(customers) + 1
        if len(test_customers) % len(customers)
        else len(test_customers) // len(customers)
    )
...
def test_new_customer_ID_is_higher_than_previous(
    authorized_client, test_customers: list[Customer]
):
    new_customer = {
...
def test_create_customer_with_personal_id_already_used(
    authorized_client, test_customers: list[Customer]
):
    new_customer = {
...
def test_updated_with_wrong_id(authorized_client, test_customers: list[Customer]):
    ...
    res = authorized_client.put(f"/customers/8888888", json=jsonable_encoder(customer))
...
def test_update_with_used_personal_id(
    authorized_client, test_customers: list[Customer]
):
    customer = copy(test_customers[0])
...
def test_update_with_blank_optional_data(
    authorized_client, test_customers: list[Customer]
):
